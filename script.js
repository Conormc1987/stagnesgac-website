...
compactLayoutStyles.textContent = `
  ...
  .teams-menu, .about-menu { position: relative; display: inline-flex; align-items: center; }
  .teams-menu > a::after, .about-menu > a::after { content: " ▾"; font-size: .72em; }
  .teams-dropdown, .about-dropdown { position:absolute; top:100%; left:50%; transform:translateX(-50%); min-width:250px; padding:10px; border-radius:12px; background:#111; border:1px solid rgba(212,175,55,.45); box-shadow:0 18px 45px rgba(0,0,0,.28); opacity:0; visibility:hidden; z-index:1000; }
  .teams-menu:hover .teams-dropdown, .teams-menu:focus-within .teams-dropdown, .about-menu:hover .about-dropdown, .about-menu:focus-within .about-dropdown { opacity:1; visibility:visible; }
  .teams-dropdown a, .about-dropdown a { display:block; padding:11px 13px; border-radius:8px; color:#fff !important; white-space:nowrap; text-decoration:none; }
  .teams-dropdown a:hover, .teams-dropdown a:focus, .about-dropdown a:hover, .about-dropdown a:focus { background:rgba(212,175,55,.16); color:#d4af37 !important; }
  @media (max-width:900px) {
    .teams-menu, .about-menu { display:block; }
    .teams-menu > a::after, .about-menu > a::after { content:""; }
    .teams-dropdown, .about-dropdown { position:static; transform:none; min-width:0; padding:4px 0 4px 16px; border:0; box-shadow:none; background:transparent; opacity:1; visibility:visible; }
    .teams-dropdown a, .about-dropdown a { padding:8px 0; }
  }
